...
echo "Running fuzztests" && time cat /dev/random | ./bin/brplot_headless_linux_debug_gcc > /dev/null && echo "Fuzz test OK" && \
...
echo "Running fuzztests" && time cat /dev/random | ./bin/brplot_headless_linux_debug_clang > /dev/null && echo "Fuzz test OK" && \
