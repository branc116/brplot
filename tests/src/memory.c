...
#define SZ (sizeof(int)*16)
  ...
  BR_FREE(ints);
/*
  br_malloc_tracker_t tracker = br_malloc_tracker_get();
  BR_ASSERT(tracker.len == 3);

  br_malloc_tracker_node_t malloc_node = tracker.arr[0];
  BR_ASSERT(malloc_node.frame_num == 0);
  BR_ASSERT(malloc_node.kind == br_malloc_tracker_event_alloc);
  BR_ASSERT(malloc_node.size == SZ);
  BR_ASSERT(malloc_node.next_nid == 1);
  br_malloc_stack_print(0);

  br_malloc_tracker_node_t realloc_node = tracker.arr[1];
  BR_ASSERT(realloc_node.frame_num == 1);
  BR_ASSERT(realloc_node.kind == br_malloc_tracker_event_realloc);
  ...
  br_malloc_stack_print(1);

  br_malloc_tracker_node_t free_node = tracker.arr[2];
  BR_ASSERT(free_node.frame_num == 3);
  BR_ASSERT(free_node.kind == br_malloc_tracker_event_freed);
  BR_ASSERT(free_node.prev_nid == 1);
  BR_ASSERT(free_node.realloc_count == 1);
  br_malloc_stack_print(2);
  ...
  br_malloc_frame();
  tracker = br_malloc_tracker_get();
  br_malloc_tracker_frames_t frames = tracker.frames;
