...
bool br_permastate_savef_plots(BR_FILE* f, br_plots_t plots) {
  ...
  if (1 != BR_FWRITE(&command, sizeof(command), 1, f))                      BR_ERROR("Failed to write command: %s", strerror(errno));
  if (1 != BR_FWRITE(&plots_len, sizeof(plots_len), 1, f))                  BR_ERROR("Failed to write plots len: %s", strerror(errno));

  if (plots_len != BR_FWRITE(plots.arr, sizeof(*plots.arr), plots_len, f))  BR_ERROR("Failed to write plots: %s", strerror(errno));
  ...
  for (int i = 0; i < plots.len; ++i) {
    ...
    if (1 != BR_FWRITE(&len, sizeof(len), 1, f))                            BR_ERROR("Failed to write data_info len i=%d: %s", i, strerror(errno));
    if (len == 0) continue;
    if (len != (int32_t)BR_FWRITE(arr, sizeof(*arr), (uint32_t)len, f))     BR_ERROR("Failed to write data_info i=%d: %s", i, strerror(errno));
    crc = br_fs_crc(arr, sizeof(*arr) * (size_t)len, crc);
  }
  if (1 != BR_FWRITE(&crc, sizeof(crc), 1, f))                              BR_ERROR("Failed to write crc %s", strerror(errno));
  goto end;
...
bool br_permastate_savef_data(BR_FILE* file, br_dagens_t const* dagens, br_data_t data) {
  ...
  if (1 != BR_FWRITE(&data.color, sizeof(data.color), 1, file))                             BR_ERRORE("Failed to write color");
  if (1 != BR_FWRITE(&data.len, sizeof(data.len), 1, file))                                 BR_ERRORE("Failed to write length");
  switch (data.kind) {
    case br_plot_kind_2d: {
      if (1 != BR_FWRITE(&data.dd.bounding_box, sizeof(data.dd.bounding_box), 1, file))     BR_ERRORE("Failed to write bounding box");
      if (1 != BR_FWRITE(&data.dd.rebase_x, sizeof(data.dd.rebase_x), 1, file))             BR_ERRORE("Failed to write rebase x");
      if (1 != BR_FWRITE(&data.dd.rebase_y, sizeof(data.dd.rebase_y), 1, file))             BR_ERRORE("Failed to write rebase y");
      if (0 != data.len) {
        if (data.len != BR_FWRITE(data.dd.xs, sizeof(*data.dd.xs), data.len, file))         BR_ERRORE("Failed to write xs");
        if (data.len != BR_FWRITE(data.dd.ys, sizeof(*data.dd.ys), data.len, file))         BR_ERRORE("Failed to write ys");
      }
    } break;
    case br_plot_kind_3d: {
      if (1 != BR_FWRITE(&data.ddd.bounding_box, sizeof(data.ddd.bounding_box), 1, file))   BR_ERRORE("Failed to write bounding box");
      if (1 != BR_FWRITE(&data.ddd.rebase, sizeof(data.ddd.rebase), 1, file))               BR_ERRORE("Failed to write rebase");
      if (0 != data.len) {
        if (data.len != BR_FWRITE(data.ddd.xs, sizeof(*data.ddd.xs), data.len, file))       BR_ERRORE("Failed to write xs");
        if (data.len != BR_FWRITE(data.ddd.ys, sizeof(*data.ddd.ys), data.len, file))       BR_ERRORE("Failed to write ys");
        if (data.len != BR_FWRITE(data.ddd.zs, sizeof(*data.ddd.zs), data.len, file))       BR_ERRORE("Failed to write zs");
      }
    } break;
    default: BR_UNREACHABLE("Data kind %d", data.kind);
  }
...
bool br_permastate_save_datas(br_str_t path_folder, br_dagens_t const* dagens, br_datas_t datas) {
  ...
  brfl_foreachv(data, datas) {
    ...
    br_str_to_c_str1(path_folder, buff);
    if (NULL == (file = BR_FOPEN(buff, "wb")))                                   BR_ERRORE("Failed to open file %s", buff);
    if (false == br_permastate_savef_data(file, dagens, data))                BR_ERROR("Failed to write data %d", data.group_id);
    BR_FCLOSE(file);
    file = NULL;
    buff[0] = '\0';
...
error:
  ...
  if (NULL != file) BR_FCLOSE(file);
  br_str_free(path_folder);
  return success;
}

static bool br_permastate_savef_plotter(BR_FILE* file, br_plotter_t* br) {
  bool success = true;
...
//  bool success      = true;
//
//  if (NULL == (f = BR_FOPEN(path_to, "wb")))                                      BR_ERROR("Failed to open a file %s: %s", path_to, strerror(errno));
...
bool br_permastate_load_plotter(BR_FILE* file, br_plotter_t* br, br_data_descs_t* desc) {
  int datas_len = 0;
  int success = true;
  (void)success;
...
bool br_permastate_load_plots(BR_FILE* file, br_plotter_t* br) {
  size_t plots_len        = 0;
  uint32_t read_crc       = 0;
...
br_permastate_status_t br_permastate_load(br_plotter_t* br_initial) {
  ...
  if (false == br_fs_cd(&path, BR_STRL("plotter.br")))                BR_ERROR("Failed to navigate to brplotter.br file.");
  {
    ...
    if (1 != BR_FREAD(&command, sizeof(command), 1, f))                  BR_ERROR("Failed to read a command: %s", strerror(errno));
  ...
  for (size_t i = 0; i < descs.len; ++i) {
    ...
    if (false == file_exists || false == br_dagen_push_file(&loaded_br.dagens, &loaded_br.groups, &descs.arr[i], BR_FOPEN(buff, "rb"))) {
