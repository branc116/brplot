...
bool brps_editor_write(br_plotter_t const* br, char const* file_name) {
  ...
  for (int i = 0; i < BR_ALL_COLLAPSABLES; ++i) {
    copy.ui.all_collapsables[i].anim_handle = brps_anim_save(br, &copy, copy.ui.all_collapsables[i].anim_handle);
  }
  ...
  LOGI("Wrote %zd bytes to %s", (size_t)BR_FTELL(file), file_name);
...
bool brps_editor_read(br_plotter_t* br, char const* file_name) {
  ...
  LOGI("Read %zd bytes from %s", (size_t)BR_FTELL(file), file_name);
  BR_FS_EOF(file);
  ...
  for (int i = 0; i < BR_ALL_COLLAPSABLES; ++i) {
    br->ui.all_collapsables[i].anim_handle = brps_anim_load(br, &copy, copy.ui.all_collapsables[i].anim_handle);
  }
