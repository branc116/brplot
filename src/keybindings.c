...
#define X(key, kind, name) static inline void name(br_plot_t* br, br_keybinding_ctrl_shift_t meta);
KEY_BINDINGS(X)
#undef X
...
#define X(key, kind, name) if (((kind) == KEY_PRESS && IsKeyPressed(key)) || ((kind) == KEY_DOWN && IsKeyDown(key))) name(br, meta);
  KEY_BINDINGS(X)
#undef X
}

static inline void br_keybinding_gui_reset(br_plot_t* br, br_keybinding_ctrl_shift_t cs) {
  if (!cs.ctrl)  br->uvZoom.x   = br->uvZoom.y = 1;
  if (!cs.shift) br->uvOffset.x = br->uvOffset.y = 0;
}

static inline void br_keybinding_clear(br_plot_t* br, br_keybinding_ctrl_shift_t cs) {
  if (cs.shift) points_groups_deinit(&br->groups);
  else          points_groups_empty(&br->groups);
}

static inline void br_keybinding_hide(br_plot_t* br, br_keybinding_ctrl_shift_t cs) {
...
static inline void br_keybinding_test_points(br_plot_t* br, br_keybinding_ctrl_shift_t cs) {
  (void)cs;
...
static inline void br_keybinding_recoil_smol(br_plot_t* br, br_keybinding_ctrl_shift_t cs) {
  (void)cs;
  br->recoil -= 0.001f;
}
static inline void br_keybinding_recoil_big(br_plot_t* br, br_keybinding_ctrl_shift_t cs) {
  (void)cs;
  br->recoil += 0.001f;
}

static inline void br_keybinding_screenshot(br_plot_t* br, br_keybinding_ctrl_shift_t cs) {
  (void)cs;
  graph_screenshot(br, "test.png"); // TODO set a sensible name
}
