...
typedef struct brui_drag_t {
  brui_drag_mode_t mode;
  int index;
  br_vec2_t point;
  br_extent_t old_ex;
  ...
  switch (ev.kind) {
    ...
    case brpl_event_touch_update: {
      ...
      if (uiw->touch_points.free_len == 3) {
        ...
        int drag_index = brui_state.drag.index;
        if (brui_state.drag.mode == brui_drag_mode_none) {
          uiw->mouse.pos = ev.pos;
          brui_state.drag.mode = brui_drag_mode_touch;
          brui_state.drag.active_resizable = bruir_find_at(&uiw->resizables, 0, BR_VEC2(new_x_mid, new_y_mid), &(br_vec2_t) {0});
          drag_index = brui_state.drag.index = brui_state.drag.active_resizable;
          brui_resizable_set_ancor(drag_index, 0, brui_ancor_none);
        }
        brui_resizable_t* r = &br_da_get(uiw->resizables, drag_index);
        ...
        //brui_state.active_resizable = bruir_find_at(&uiw->resizables, 0, ev.touch.pos, &(br_vec2_t) {0});
      ...
      if (to_remove >= 0) {
        ...
        if (uiw->touch_points.free_len == 0) {
          ...
          if (diff < 0.5) {
            ...
            brui_resizable_mouse_releasel(point.pos);
          }
        }
      }
    } break;
...
void brui_begin(void) {
  ...
  el->pos = TOP.cur_pos;
  el->font_size = BR_THEME.font_size;
  el->z = 1;
  ...
  TOP.is_active = 0 == brui_state.drag.active_resizable;
...
void brui_scroll_move(float value) {
  brui_resizable_t* res = &br_da_get(brui_state.uiw->resizables, TOP.cur_resizable);
...
static bool brui_snap_area(brui_ancor_t ancor, br_bb_t bb, br_vec2_t mouse_pos, int r_id, int sibling_id, float light_f) {
  bool is_in = false;
  if (br_col_vec2_bb(bb, mouse_pos)) {
    brui_state.drag.point = mouse_pos;
    if (brui_state.len == 0) {
...
static bool brui_snap_areas(br_vec2_t mouse_pos, int r_id, bruirs_t* rs) {
  ...
  brui_resizable_t* r = &br_da_get(*rs, r_id);
  ...
  brtr_stack_el_t* el = brtr_state_push();
    ...
    brfl_foreach(i, *rs) {
      if (i == 0) continue;
      if (i == r_id) continue;
      brui_resizable_t* sib = &br_da_get(*rs, i);
      ...
      br_bb_t snap_bb = { 0 };
      {
        ...
        if (brui_snap_area(brui_ancor_top, snap_bb, mouse_pos, r_id, i, light_f)) return true;
        snap_bb.min.y += sex.height;
        snap_bb.max.y += sex.height;
        if (brui_snap_area(brui_ancor_bottom, snap_bb, mouse_pos, r_id, i, light_f)) return true;
      }
      {
        ...
        if (brui_snap_area(brui_ancor_left, snap_bb, mouse_pos, r_id, i, light_f)) return true;
        snap_bb.min.x += sex.width;
        snap_bb.max.x += sex.width;
        if (brui_snap_area(brui_ancor_right, snap_bb, mouse_pos, r_id, i, light_f)) return true;
      }
    }
    ...
    brui_snap_area(brui_ancor_all,          BR_BB(center.x - halfs, center.y - halfs, center.x + halfs, center.y + halfs), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_right,        BR_BB(center.x + halfs + pad, center.y - halfs, center.x + halfs + pad + halfs, center.y + halfs), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_left,         BR_BB(center.x - halfs - pad - halfs, center.y - halfs, center.x - pad - halfs, center.y + halfs), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_top,          BR_BB(center.x - halfs, center.y - halfs - pad - halfs, center.x + halfs, center.y - halfs - pad), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_bottom,       BR_BB(center.x - halfs, center.y + halfs + pad, center.x + halfs, center.y + halfs + pad + halfs), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_right_top,    BR_BB(center.x + halfs + pad, center.y - halfs - pad - halfs, center.x + halfs + pad + halfs, center.y - halfs - pad), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_left_top,     BR_BB(center.x - halfs - pad - halfs, center.y - halfs - pad - halfs, center.x - halfs - pad, center.y - halfs - pad), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_right_bottom, BR_BB(center.x + halfs + pad, center.y + halfs + pad, center.x + halfs + pad + halfs, center.y + halfs + pad + halfs), mouse_pos, r_id, 0, light_f);
    brui_snap_area(brui_ancor_left_bottom,  BR_BB(center.x - halfs - pad - halfs, center.y + halfs + pad, center.x - halfs - pad, center.y + halfs + pad + halfs), mouse_pos, r_id, 0, light_f);
  brtr_state_pop();
...
void brui_resizable_update(bruirs_t* rs, br_extent_t viewport) {
  ...
  if (brui_state.drag.mode == brui_drag_mode_none) {
    brui_state.drag.active_resizable = bruir_find_at(rs, 0, brui_state.uiw->mouse.pos, &(br_vec2_t) {0});
  }

  brfl_foreach(i, *rs) {
    brui_resizable_t* res = &br_da_get(*rs, i);
  ...
  brfl_foreach(i, *rs) {
    brui_resizable_t* r = &br_da_get(*rs, i);
    brui_resizable_t* parent = &br_da_get(*rs, r->parent);
...
static brui_drag_t brui_drag_mode_if_clicked(bruirs_t* rs, br_vec2_t mouse_pos, bool ctrl_down) {
  if (brui_state.drag.active_resizable < 0) return (brui_drag_t) { .mode = brui_drag_mode_none };
  brui_drag_t new_mode = { 0 };
  brui_resizable_t* hovered = &br_da_get(*rs, brui_state.drag.active_resizable);
  bool title_shown = brui_state.drag.active_resizable == 0 ? false : BRUI_ANIMF(hovered->title_height_ah) > 0.1f;
  if ((ctrl_down || title_shown)) {
    float slack = 20;
    if (brui_state.drag.active_resizable != 0) {
      br_extent_t ex = BRUI_ANIMEX(hovered->cur_extent_ah);
      new_mode.index = brui_state.drag.active_resizable;
      new_mode.point = mouse_pos;
      if (title_shown) new_mode.mode = brui_drag_mode_move;
      else {
        br_vec2_t local_pos = brui_resizable_local(rs, brui_state.drag.active_resizable, mouse_pos);
        if      (local_pos.x < slack)                    new_mode.mode |= brui_drag_mode_left;
        else if (local_pos.x > (float)ex.width - slack)  new_mode.mode |= brui_drag_mode_right;
        if      (local_pos.y < slack)                    new_mode.mode |= brui_drag_mode_top;
        else if (local_pos.y > (float)ex.height - slack) new_mode.mode |= brui_drag_mode_bottom;
        if      (new_mode.mode == brui_drag_mode_none)   new_mode.mode  = brui_drag_mode_move;
      }
      if (!(new_mode.mode & brui_drag_mode_move)) {
        brui_resizable_t* parent = &br_da_get(brui_state.uiw->resizables, hovered->parent);
        while (!(new_mode.mode & brui_drag_mode_move) && parent->tag == brui_resizable_tag_ancor_helper) {
          if (hovered->ancor & brui_ancor_top) {
            if (new_mode.mode & brui_drag_mode_bottom) new_mode.mode = brui_drag_mode_move;
          } else if (hovered->ancor & brui_ancor_bottom) {
            if (new_mode.mode & brui_drag_mode_top) new_mode.mode = brui_drag_mode_move;
          } else if (hovered->ancor & brui_ancor_left) {
            if (new_mode.mode & brui_drag_mode_right) new_mode.mode = brui_drag_mode_move;
          } else if (hovered->ancor & brui_ancor_right) {
            if (new_mode.mode & brui_drag_mode_left) new_mode.mode = brui_drag_mode_move;
          }
          hovered = parent;
          parent = &br_da_get(brui_state.uiw->resizables, hovered->parent);
        }
      }
      new_mode.old_ex = BRUI_ANIMEX(hovered->cur_extent_ah);
    }
  }
  return new_mode;
}
...
void brui_resizable_mouse_move(bruirs_t* rs, br_vec2_t mouse_pos) {
  if (brui_state.drag.mode == brui_drag_mode_none) return;

  int drag_index = brui_state.drag.index;
  brui_resizable_t* r = &br_da_get(*rs, drag_index);
  br_extent_t new_extent = brui_state.drag.old_ex;
  if (brui_state.drag.mode == brui_drag_mode_touch) {
    return;
  } else if (brui_state.drag.mode == brui_drag_mode_move) {
    new_extent.pos = br_vec2_sub(brui_state.drag.old_ex.pos, br_vec2_sub(brui_state.drag.point, mouse_pos));
    if (r->ancor != brui_ancor_none) {
      if (br_vec2_len2(br_vec2_sub(new_extent.pos, brui_state.drag.old_ex.pos)) > 10*10) {
        brui_state.drag.old_ex.pos = brui_state.drag.point = mouse_pos;
        brui_resizable_set_ancor(drag_index, 0, brui_ancor_none);
      }
    }
    ...
    brpl_pointer_kind_set(&brui_state.uiw->pl, brpl_pointer_move);
  } else {
    if (brui_state.drag.mode & brui_drag_mode_left) {
      float dif = brui_state.drag.point.x - mouse_pos.x;
      new_extent.width  = brui_state.drag.old_ex.width  + dif;
      new_extent.x      = brui_state.drag.old_ex.x      - dif;
    } else if (brui_state.drag.mode & brui_drag_mode_right) {
      float dif = brui_state.drag.point.x - mouse_pos.x;
      new_extent.width  = brui_state.drag.old_ex.width  - dif;
    }
    if (brui_state.drag.mode & brui_drag_mode_top) {
      float dif = brui_state.drag.point.y - mouse_pos.y;
      new_extent.y      = brui_state.drag.old_ex.y      - dif;
      new_extent.height = brui_state.drag.old_ex.height + dif;
    } else if (brui_state.drag.mode & brui_drag_mode_bottom) {
      float dif = brui_state.drag.point.y - mouse_pos.y;
      new_extent.height = brui_state.drag.old_ex.height - dif;
    }

    brpl_pointer_kind_set(&brui_state.uiw->pl, brpl_pointer_resize);
  }
...
void brui_resizable_mouse_pressl(bruirs_t* rs, br_vec2_t mouse_pos, bool ctrl_down) {
  if (brui_state.drag.active_resizable < 0) return;
  brui_resizable_t* hovered = &br_da_get(*rs, brui_state.drag.active_resizable);
  bool title_shown = brui_state.drag.active_resizable == 0 ? false : BRUI_ANIMF(hovered->title_height_ah) > 0.1f;
  ...
  if ((ctrl_down || title_shown)) {
    ...
    if (brui_state.drag.active_resizable != 0) {
      br_extent_t ex = BRUI_ANIMEX(hovered->cur_extent_ah);
      brui_state.drag.index = brui_state.drag.active_resizable;
      brui_state.drag.point = mouse_pos;
      ...
      if (!(new_mode & brui_drag_mode_move)) {
        ...
        while (!(new_mode & brui_drag_mode_move) && parent->tag == brui_resizable_tag_ancor_helper) {
          ...
          brui_state.drag.index = hovered->parent;
          brui_state.drag.active_resizable = hovered->parent;
          hovered = parent;
          parent = &br_da_get(brui_state.uiw->resizables, hovered->parent);
          BRUI_LOGI("Hover ancor: %s, parent->tag = %d", brui_ancor_to_str(hovered->ancor).str, parent->tag);
        }
      }
      brui_state.drag.mode = new_mode;
      brui_state.drag.old_ex = BRUI_ANIMEX(hovered->cur_extent_ah);
...
bool brui_resizable_mouse_scroll_px(bruirs_t* rs, br_vec2_t delta) {
  brui_resizable_t hovered = rs->arr[brui_state.drag.active_resizable];
  float new = 0.f;
...
static void brui_resizable_decrement_z(bruirs_t* rs, brui_resizable_t* res) {
  ...
  for (int i = 0; i < c.len; ++i) {
    ...
    brui_resizable_t* sibling = &br_da_get(*rs, sibling_index);
...
static bool brui_resizable_increment_z(bruirs_t* rs, int resizable_handle) {
  brui_resizable_t* res = &br_da_get(*rs, resizable_handle);
...
void brui_resizable_move_on_top(bruirs_t* rs, int r_handle) {
  brui_resizable_t* res = &br_da_get(*rs, r_handle);
  while (res->parent != 0) {
    r_handle = res->parent;
    res = &br_da_get(*rs, r_handle);
  }
...
int brui_resizable_sibling_max_z(int id) {
  if (id == 0) return 0;
  brui_resizable_t* res = &br_da_get(brui_state.uiw->resizables, id);
  ...
  for (int i = 0; i < siblings.len; ++i) {
    ...
    brui_resizable_t* sibling = &br_da_get(brui_state.uiw->resizables, sibling_id);
...
int brui_resizable_active(void) {
  return brui_state.drag.active_resizable;
}
...
static void brui_resizable_set_ancor(int res_id, int sibling_id, brui_ancor_t ancor) {
  ...
  brui_resizable_t* parent = &br_da_get(*rs, parent_id);
  brui_resizable_t* sibling = &br_da_get(*rs, sibling_id);
  br_extent_t ex = BRUI_ANIMEX(res->cur_extent_ah);
  ...
  if (0 != sibling_id) {
    ...
    res = &br_da_get(brui_state.uiw->resizables, res_id);
    sibling = &br_da_get(brui_state.uiw->resizables, sibling_id);
    res->parent = new_id;
    ...
    brui_state.drag.point = brui_state.uiw->mouse.pos;
    brui_state.drag.old_ex = BRUI_ANIMEX(res->cur_extent_ah);
    brui_resizable_check_parents(&brui_state.uiw->resizables);
    ...
    if (parent->tag == brui_resizable_tag_ancor_helper) {
      ...
      brui_resizable_t* gparent = &br_da_get(*rs, gp);
      res->parent = gp;
      br_anim_rebase(&brui_state.uiw->anims, res->cur_extent_ah, rebase);
      brui_state.drag.old_ex = ex;
      ...
      brui_state.drag.old_ex = br_animex_get_target(&brui_state.uiw->anims, res->cur_extent_ah);
...
brui_resizable_t* brui_resizable_push(int id) {
  bruirs_t* rs = &brui_state.uiw->resizables;
  brui_resizable_t* res = &br_da_get(*rs, id);
  ...
  while (ancor_parent_id > 0) {
    brui_resizable_t* parent = &br_da_get(*rs, ancor_parent_id);
    if (parent->tag == brui_resizable_tag_ancor_helper) {
      max_z_id = res->parent;
  ...
  TOP.start_z = TOP.z;
  TOP.is_active = id == brui_state.drag.active_resizable;
  TOP.hide_border = true;
  TOP.hide_bg = true;
  brui_push();
  TOP.cur_resizable = id;
...
void brui_resizable_pop(void) {
  ...
  brui_resizable_t* res = &br_da_get(*rs, cur_res);
  br_extent_t ex = BRUI_ANIMEX(res->cur_extent_ah);
  ...
  if (cur_res == brui_state.drag.active_resizable && brui_state.drag.mode == brui_drag_mode_move) {
...
br_vec2_t brui_resizable_to_global(int resizable_handle, br_vec2_t pos) {
  ...
  brui_resizable_t* r = &br_da_get(brui_state.uiw->resizables, resizable_handle);
  br_extent_t ex = BRUI_ANIMEX(r->cur_extent_ah);
...
static int bruir_find_at(bruirs_t* rs, int index, br_vec2_t loc, br_vec2_t* out_local_pos) {
  ...
  int best_z = 0;
  brfl_foreach(i, *rs) {
    brui_resizable_t* c = &br_da_get(*rs, i);
