...
br_plotter_t* br_plotter_malloc(void) {
  ...
  *br = (br_plotter_t) {
    ...
    .ui = {
      ...
      .min_sampling = 0.001f,
      .cull_min = 0.01f,
      .default_grid_line_thickenss = 1.f,
      .dark_theme = true,
    },
  };
#if BR_HAS_HOTRELOAD
...
static br_plot_t br_plot_2d(br_anims_t* anims, float grid_line_thickness) {
  br_plot_t plot = {
  ...
  float def_zoom = 1.f;
#endif
  plot.dd.zoom_ah = br_anim2d_new(anims, BR_VEC2D(def_zoom, def_zoom), BR_VEC2D(def_zoom, def_zoom));
  plot.dd.offset_ah = br_anim2d_new(anims, BR_VEC2D(0.f, 0.f), BR_VEC2D(0.f, 0.f));
  return plot;
}
...
void br_plotter_init(br_plotter_t* br) {
  ...
  if (read_editor) {
    if (0 != br->ui.font_path_id)    brtr_font_load(brsp_get(br->uiw.sp, br->ui.font_path_id));
    if (0 != br->ui.project_path_id) brps_project_read(br, brsp_cstr(&br->uiw.sp, br->ui.project_path_id));
  } else {
    br->ui.menu_extent_handle = brui_resizable_new(&br->uiw.resizables, BR_EXTENT(10, 40, 160, (float)br->uiw.pl.viewport.height/2.f), 0);
            ...
            switch (ev.key) {
              ...
              case BR_KEY_S: {
                if (br->uiw.key.ctrl_down) {
                  brps_project_write(br, brsp_cstr(&br->uiw.sp, br->ui.project_path_id));
                }
              } break;
            }
...
//                }
      ...
      case brpl_event_mouse_move: {
        ...
        if (br->action.active == br_plotter_entity_plot_2d) {
          ...
          if (horizontal_factor > 0.94f) br_anim3_set(anims, plot->ddd.eye_ah, br_vec3_add(rotated_up,    target));
          else                           br_anim3_set(anims, plot->ddd.eye_ah, br_vec3_add(rotated_right, target));
        }
      } break;
      case brpl_event_mouse_scroll: {
        ...
        if (br->hovered.active == br_plotter_entity_plot_2d) {
          ...
          br_extent_t ex = brui_resizable_cur_extent(plot->resizable_handle);
          br_vec2_t zoom = BR_VEC2(-ev.vec.y*2, -ev.vec.y*2);
          ...
          br_vec3_t eye = br_anim3(anims, plot->ddd.eye_ah);
          br_vec3_t target = br_anim3(anims, plot->ddd.target_ah);
          br_vec3_t zeroed = br_vec3_sub(eye, target);
          br_vec3_t zero_dir = br_vec3_normalize(zeroed);
          float len = br_vec3_len(zeroed);
          len /= mw_scale;
          br_anim3_set(anims, plot->ddd.eye_ah, br_vec3_add(target, br_vec3_scale(zero_dir, len)));
        }
      } break;
      case brpl_event_frame_next: {
        ...
        for (int j = 0; j < br->plots.len; ++j) {
          ...
          if (plot.resizable_handle == brui_resizable_active()) {
            switch (plot.kind) {
        ...
        if (br->action.active == br_plotter_entity_none) {
          if (found_plot > -1) {
            ...
            if (br->hovered.active == br_plotter_entity_plot_2d) {
              ...
              br_anim3_set(anims, plot->ddd.target_ah, br_vec3_add(target, delta));
              br_anim3_set(anims, plot->ddd.eye_ah, br_vec3_add(eye, delta));
            }
          } else {
...
void br_plotter_switch_2d(br_plotter_t* br) {
  for (int i = 0; i < br->plots.len; ++i) {
    ...
    brui_resizable_show(br->plots.arr[i].resizable_handle, true);
    brui_resizable_move_on_top(&br->uiw.resizables, br->plots.arr[i].resizable_handle);
    return;
  }
...
int br_plotter_add_plot_2d(br_plotter_t* br) {
  ...
  br_plot_t plot = br_plot_2d(&br->uiw.anims, br->ui.default_grid_line_thickenss);
  br_plot_create_texture(&plot, ex);
  plot.resizable_handle = brui_resizable_new2(&br->uiw.resizables, ex, 0, (brui_resizable_t) { .tag = 100, .title_enabled = true });
#if defined(__EMSCRIPTEN__)
  brui_resizable_maximize(plot.resizable_handle, true);
#endif
  plot.menu_resizable_handle = brui_resizable_new2(&br->uiw.resizables, BR_EXTENT(0, 0, 300, ex.height), plot.resizable_handle, (brui_resizable_t) { .tag = 101 });
  brui_resizable_show(plot.menu_resizable_handle, false);
  plot.legend_resizable_handle = brui_resizable_new2(&br->uiw.resizables, BR_EXTENT(ex.width - 110, 10, 100, 60), plot.resizable_handle, (brui_resizable_t) { .tag = 102 });
...
int br_plotter_add_plot_3d(br_plotter_t* br) {
  ...
  br_plot_create_texture(&plot, ex);
  plot.resizable_handle = brui_resizable_new(&br->uiw.resizables, ex, 0);
  plot.menu_resizable_handle = brui_resizable_new(&br->uiw.resizables, BR_EXTENT(0, 0, 300, ex.height), plot.resizable_handle);
  brui_resizable_show(plot.menu_resizable_handle, false);
  plot.legend_resizable_handle = brui_resizable_new2(&br->uiw.resizables, BR_EXTENT(ex.width - 110, 10, 100, 60), plot.resizable_handle, (brui_resizable_t) { 0 });
  br_da_push_t(int, (br->plots), plot);
  return br->plots.len - 1;
}
