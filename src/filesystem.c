...
bool br_fs_read_internal(const char* path, br_str_t* out_content, const char* file_name, int line) {
  ...
  if (NULL == (file = BR_FOPEN(path, "rb")))                                 BR_ERROR("[%s:%d] Failed to open file %s: %s", file_name, line, path, strerror(errno));
  if (-1 == BR_FSEEK(file, 0, SEEK_END))                                     BR_ERROR("[%s:%d] Failed to seek file %s: %s", file_name, line, path, strerror(errno));
  if (-1 == (size = BR_FTELL(file)))                                         BR_ERROR("[%s:%d] Failed to get the file size %s: %s", file_name, line, path, strerror(errno));
  if (-1 == BR_FSEEK(file, 0, SEEK_SET))                                     BR_ERROR("[%s:%d] Failed to get seek set file %s: %s", file_name, line, path, strerror(errno));
  out_content->len = 0;
  ...
  if (size != (long)BR_FREAD(out_content->str, 1, (size_t)size, file))       BR_ERROR("[%s:%d] Failed to read file %s: %s", file_name, line, path, strerror(errno));
